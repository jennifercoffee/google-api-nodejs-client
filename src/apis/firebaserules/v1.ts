    ...
    releases: {
      ...
      getExecutable(
        ...
        createAPIRequest(parameters, callback!);
          ...
          * optionally be filtered by `Release` name, `Ruleset` name,
 ...
 * @memberOf! firebaserules(v1)
 * @type object
 * @property {firebaserules(v1).Arg[]} args The list of `Arg` values to match. The order in which the arguments are provided is the order in which they must appear in the function invocation.
 * @property {string} function The name of the function.  The function name must match one provided by a service declaration.
 ...
 * @memberOf! firebaserules(v1)
 * @type object
 * @property {string} nextPageToken The pagination token to retrieve the next page of results. If the value is empty, no further results remain.
 ...
 * @property {any} value The result is an actual value. The type of the value must match that of the type declared by the service.
 */
/**
 * @typedef Ruleset
 * @memberOf! firebaserules(v1)
 * @type object
 * @property {string} createTime Time the `Ruleset` was created. Output only.
 * @property {string} name Name of the `Ruleset`. The ruleset_id is auto generated by the service. Format: `projects/{project_id}/rulesets/{ruleset_id}` Output only.
 ...
 * @property {firebaserules(v1).FunctionMock[]} functionMocks Optional function mocks for service-defined functions. If not set, any service defined function is expected to return an error, which may or may not influence the test outcome.
 ...
 * @property {any} resource Optional resource value as it appears in persistent storage before the request is fulfilled.  The resource type depends on the `request.path` value.
 */
/**
 * @typedef TestResult
 * @memberOf! firebaserules(v1)
 * @type object
 * @property {string[]} debugMessages Debug messages related to test execution issues encountered during evaluation.  Debug messages may be related to too many or too few invocations of function mocks or to runtime errors that occur during evaluation.  For example: ```Unable to read variable [name: &quot;resource&quot;]```
 * @property {firebaserules(v1).SourcePosition} errorPosition Position in the `Source` or `Ruleset` where the principle runtime error occurs.  Evaluation of an expression may result in an error. Rules are deny by default, so a `DENY` expectation when an error is generated is valid. When there is a `DENY` with an error, the `SourcePosition` is returned.  E.g. `error_position { line: 19 column: 37 }`
 * @property {firebaserules(v1).FunctionCall[]} functionCalls The set of function calls made to service-defined methods.  Function calls are included in the order in which they are encountered during evaluation, are provided for both mocked and unmocked functions, and included on the response regardless of the test `state`.
 ...
 * @memberOf! firebaserules(v1)
 * @type object
 * @property {firebaserules(v1).Source} source Optional `Source` to be checked for correctness.  This field must not be set when the resource name refers to a `Ruleset`.
 ...
 * @memberOf! firebaserules(v1)
 * @type object
 * @property {firebaserules(v1).Issue[]} issues Syntactic and semantic `Source` issues of varying severity. Issues of `ERROR` severity will prevent tests from executing.
 * @property {firebaserules(v1).TestResult[]} testResults The set of test results given the test cases in the `TestSuite`. The results will appear in the same order as the test cases appear in the `TestSuite`.
 */
/**
 * @typedef TestSuite
 * @memberOf! firebaserules(v1)
